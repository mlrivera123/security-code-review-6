...
const controller = {
    ...
    getMemberActivity: {
        ...
        async query({data, options}) {
            ...
            const events = await db.knex('members_click_events')
                .where('member_id', targetId)
                .limit(limit)
                .orderBy('created_at', 'desc');
            return {events, member_id: targetId};
        }
    },
    ...
    findByFilter: {
        ...
        async query({data, options}) {
            const browseOptions = Object.assign({}, options, {
                filter: data.filter
            });
            return membersService.api.memberBREADService.browse(browseOptions);
        }
    }
};
