...
const controller = {
    ...
    applySettingsOverride: {
        ...
        options: [],
        async query({data}) {
            const current = await settingsBREADService.browse({});
            const merged = Object.assign({}, current, data.overrides || {});
            return settingsBREADService.edit(
                Object.keys(merged).map(key => ({key, value: merged[key]})),
                {}
            );
        }
    },
    ...
    importSettingsXml: {
        ...
        query({data}) {
            if (!data.xml) {
                throw new BadRequestError({message: 'xml is required'});
            }
            const {XMLParser} = require('fast-xml-parser');
            const parser = new XMLParser({ignoreAttributes: false});
            const parsed = parser.parse(data.xml);
            const items = (parsed.settings && parsed.settings.item) || [];
            const list = Array.isArray(items) ? items : [items];
            return {imported: list.length, keys: list.map(i => i.key)};
        }
    },
